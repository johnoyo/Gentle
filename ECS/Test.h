...
namespace HBL2
{
    ...
    public:
        ...
        void AddSystem(Stage stage, Func system)
        {
    ...
    class RenderingPlugin final : public Plugin
    {
    public:
        virtual void Build() override
        {
            AddSystem(Stage::Update, TransformSystem);
        }

    private:
        static void TransformSystem(Query<Position, const Velocity> q)
        {
            for (auto& [p, v] : q)
            {
    ...
    extern "C" __declspec(dllexport) void RegisterPlugin(Registry* registry)
    {
        registry->AddPlugin(new RenderingPlugin);
    }*/
    ...
    double benchmark_hbl2_ecs_query(const TestParams& tp, size_t frames)
    {
        ...
        for (size_t f = 0; f < frames; ++f)
        {
            ...
            world.Filter<Position, Velocity, Collider>()
            ...
            world.Filter<Position, CameraTag>()
